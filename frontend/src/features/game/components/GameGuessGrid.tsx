...
export const GameGuessGrid = ({ guess, playerGuessesArray, letterStatusesArray }: GameGuessGridProps) => {
  return (
    <Stack component="section" sx={{ gap: 1, width: "100%", maxWidth: 360 }}>
      {getVisibleGuessRows(guess, playerGuessesArray).map((rowGuess: string, rowIndex: number) => (
        <Stack
          key={rowIndex}
          sx={{ display: "grid", gridTemplateColumns: `repeat(${WORD_LENGTH}, minmax(0, 1fr))`, gap: 1 }}
        >
          {Array.from(rowGuess).map((letter, colIndex) => {
            const isCurrentRow = rowIndex === playerGuessesArray.length;
            const backgroundColor = getLetterStatusBackgroundColor(rowIndex, colIndex, letterStatusesArray);
            return (
              <Box
                key={colIndex}
